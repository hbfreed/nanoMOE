...
wandb_run_name = f'moe-{num_experts}-{num_experts_per_tok}-{n_head}h-{n_layer}l-{expert_sizes_str}'

# Set output directory based on MoE configuration
if use_moe:
    out_dir = f'out-wikitext/{num_experts}_{num_experts_per_tok}_{n_head}h_{n_layer}l-{expert_sizes_str}'
else:
